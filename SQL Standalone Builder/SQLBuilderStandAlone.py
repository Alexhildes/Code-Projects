...
#09.03.2020
#Rewrote to accommodate a Standalone .exe for the service technicians.
...
#Files that only are .xlsx files
file_list = []
for file in os.listdir(cur_dir):
    if file.endswith(".xlsx"):
        file_list.append(file)

#Lets see file list and print it
print(str(len(file_list)) + ' Files ready to be converted to SQL Insert Scripts')
print(file_list)
print('')
...
for file in os.listdir(cur_dir):
    if file.endswith(".xlsx"):
        print('Read to convert ' + file)

        #Loads excel file - must be in the correct format
        bimData = load_workbook(cur_dir + '/' + file)

        #Opens blank sql file
        with open(cur_dir + '/' + file + '.sql','w') as f:
            ...
            for i in range (2, maxRows):
                ...
                elif activeSheet.cell(row=i, column=1).value == None:
                    print('ID is blank at Row ' + str(i) + ', ensure each Record has an ID. Consult Points Lists or dbo.device SQL table and try again')
                    #Exit the script
                    input()
                    sys.exit()
        ...
        print(file.replace('.xlsx','') + " SQL Insert File Created")
        print(".......................")
        print('')
